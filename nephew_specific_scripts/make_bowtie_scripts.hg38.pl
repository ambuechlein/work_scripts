...
while(my $f = readdir($din)){
  ...
  my $command1 = "bowtie2 -p 8 --mm -k 20 -S ${fullout}${prefix}.sam -x /nfs/labs/nephew/databases/gencode_v27/genome -U $file";
  my $command2 = "samtools view -@ 8 -bS -F 256 -o ${fullout}${prefix}.bam ${fullout}${prefix}.sam";
  my $command3 = "perl /nfs/labs/nephew/scripts/filterSam.pl ${fullout}${prefix}.bam | samtools view -h -bS -o ${fullout}${prefix}.best.bam - ";
  ...
  my $command6 = "/home/abuechle/bin/bedtools2/bin/genomeCoverageBed -bg -ibam ${fullout}${prefix}.sorted.bam -g /nfs/labs/nephew/databases/gencode_v27/chrSizes.tsv > ${fullout}${prefix}.bedgraph";
  my $command7 = "perl /nfs/labs/nephew/scripts/singleMappedBam.random.pl ${fullout}${prefix}.sorted.bam | samtools view -@ 8 -bS - > ${fullout}${prefix}.s.bam";
  my $command8 = "samtools sort -@ 8 -m 4G -O bam -o ${fullout}${prefix}.s.sorted.bam ${fullout}${prefix}.s.bam";
  my $command9 = "/home/abuechle/bin/bedtools2/bin/genomeCoverageBed -bg -ibam ${fullout}${prefix}.s.sorted.bam -g /nfs/labs/nephew/databases/gencode_v27/chrSizes.tsv > ${fullout}${prefix}.single.bedgraph";
