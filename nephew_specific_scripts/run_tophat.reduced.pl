...
while(my $f = readdir($din)){
  ...
  my $command = 'tophat2 -o '.$outdir.' --transcriptome-index=/nfs/labs/nephew/human_databases/gencode19_with_contaminates/transcriptome-index -p 8 --b2-very-sensitive --read-edit-dist 2 --max-multihits 100 --library-type fr-secondstrand /nfs/labs/nephew/human_databases/gencode19_with_contaminates/genome ' . $file;
...
#  print $sh '/nfs/bio/sw/bin/samtools view ' .$outdir. '/accepted_hits_sorted.bam > ' . $outdir . '/accepted_hits_sorted.sam' . ' || { echo "samtools view failed"; exit 1; }'."\n";
  ...
  system("qsub -q cluster -pe pe_slots 8 -wd $cwd tophat.$f.sh");
}
exit;
