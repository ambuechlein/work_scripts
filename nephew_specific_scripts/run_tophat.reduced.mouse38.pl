...
while(my $f = readdir($din)){
  ...
  my $command = 'tophat2 -o '.$outdir.' --transcriptome-index=/nfs/labs/nephew/human_databases/gencode_m10/transcriptome-index -p 8 --b2-very-sensitive --read-edit-dist 2 --max-multihits 100 --library-type fr-secondstrand /nfs/labs/nephew/human_databases/gencode_m10/genome ' . $file;
