...
for (i in names(samC.70)) {
  ...
  plot(samC.70[[i]], nrC.70[[i]], main=i, xlab="All Reads", ylab="Nonredundant Reads", pch=19)
  abline(lm(nrC.70[[i]]~samC.70[[i]]), col="red")
  plot(samC.70[[i]], nrC.70[[i]], main=i, xlab="All Reads", ylab="Nonredundant Reads", pch=19, xlim=c(0,3000), ylim=c(0,3000))
  abline(lm(nrC.70[[i]]~samC.70[[i]]), col="red")
  points(pom$AllReads, pomNR$Nonredundant, pch=19, col="blue")
  dev.off()
}
#dev.off()
