...
while(my $file = readdir($din)){
  ...
  my $bg = $dir . '/'. $file;
  my $color = 'black';
  print $out "perl /home/drusch/bin/bedGraph2Jbrowse2.pl $jb $bg /nfs/bio/db/Homo_sapien/gencode19_with_contaminates/chrSizes.tsv \"$label\" \"$label\" linear local 0 \"black\" 30 50\n";
