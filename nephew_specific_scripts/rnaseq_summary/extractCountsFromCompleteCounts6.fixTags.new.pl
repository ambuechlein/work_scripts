...
use strict;
use warnings;
use lib '/home/drusch/jcviBin/bioinformatics/utilities/';
use lib '/home/drusch/CPAN/lib/lib';
use lib '/home/drusch/CPAN/lib/lib/arch';
use lib '/home/drusch/lib/perl5/lib/perl/5.14.2/';
use lib '/home/drusch/jcviBin/bioinformatics/utilities/';
use lib '/home/drusch/jcviBin/frameshiftCorrection/trunk/FrameshiftCorrection/';
#use FileUtil;
...
open(my $gfh,$gtf) or die "Can't open $gtf: $!\n";
while (<$gfh>) {
	chomp;
        next if($_ =~ /^#/);
	my @d = split /\t/;
...
open(my $ofh,$order) or die "Can't open $order: $!\n";
while (<$ofh>) {
	chomp;
	my @d = split /\t/;
	if ($d[1]) {
...
open(my $lfh, $list) or die "Can't open $list: $!\n";
while (<$lfh>) {
	chomp;
	my $f = $_;
	print STDERR $f,"\n";
...
#	my $fh = FileUtil::openFileHandle("$f");
        open(my $fh, $f =~ /.gz(ip)?$/ ? "zcat $f |" : $f =~ /.bz(ip)?2$/ ? "bzcat $f |" : $f) || die("Open error: $f");
	while (<$fh>) {
		chomp;
                next if($_ =~ /^\@/o);
		my $l = $_;
...
open(my $sfho, ">$summaryOut") or die "Can't open $summaryOut: $!\n";
my $printo = join "\t","Sample",@altOrder;
print $sfho $printo,"\n";

$printo = join "\t","FilePrefix",@order;
print $sfho $printo,"\n";
### total hits;
my @l = ();
push @l,"All Reads";
...
push @l,"Valid Reads";
foreach my $i (@order) {
        push @l,0+$totalReads{$i}-$noHit{$i}-$typeHist{$i}{2};
}
$printo = join "\t",@l;
print $sfho $printo,"\n";
...
open(my $lfho,">$baseHistOut.length.vs.type.hist") or die "can't open $baseHistOut.length.vs.type.hist: $!\n";
@l = ();
...
foreach my $i (keys %ambigMap) {
	...
	open(my $lfho, ">$baseHistOut.$i.ambiguous.links") or die "can't open $baseHistOut.$i.ambiguous.links: $!\n";
...
open($lfho, ">$baseHistOut.abundance.cnts") or die "Can't open $baseHistOut.abundance.cnts: $!\n";
...
open($lfho,">$baseHistOut.gene.cnts") or die "Can't open $baseHistOut.gene.cnts: $!\n";
{
	my %tmp;
