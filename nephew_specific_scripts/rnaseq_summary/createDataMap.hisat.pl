...
#  my $col2 = "$options{tophat}/$col1.fastq.nonredundant.fasta_thout/accepted_hits_sorted.bam";
  my $col2 = "$options{tophat}/$col1.$post.fasta_hisat/accepted_hits.sorted.bam";
...
#  my $col8 = "$options{tophat}/$col1.fastq.nonredundant.fasta_thout/unmapped.bam";
  my $col8 = "$options{tophat}/$col1.$post.fasta_hisat/unmapped.fsa";
  ...
  print $sh "perl /nfs/labs/nephew/scripts/rnaseq_summary/combinedNewNephewDatasets2.fixTags.hisat.pl $pwd/$col1.$options{out}" . ' || { echo "combine failed"; exit 1; }'."\n";
