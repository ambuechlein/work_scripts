...
unless(-e $bedGraphFile && -s $bedGraphFile) {
...
#	my $tfh = FileUtil::openFileHandle($bedGraphFile);
        ...
        open(my $tfh, $bedGraphFile) or die "Can't open $bedGraphFile: $!\n";
	while (<$tfh>) {
		if (/^track/i) {
			print $ofh $_;
		} else {
...
open(my $ofh, ">$currentTrackFile") or die "Can't open $currentTrackFile: $!\n";
open(my $fh, $oldTrackFile) or die "Can't open $oldTrackFile: $!\n";
