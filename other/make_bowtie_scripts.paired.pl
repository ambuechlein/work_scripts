...
while(my $f = readdir($din)){
  ...
  my $command1 = "/N/u/abuechle/Karst/miniconda3/bin/bowtie2 -p 8 --mm -k 20 --very-sensitive -S ${fullout}${prefix}.sam -x /N/dc2/projects/cgbgsf/Rnor_6.0/genome -1 $file1 -2 $file2";
...
#  my $command3 = "perl /N/u/abuechle/Karst/bin/filterSam.pl ${fullout}${prefix}.bam | samtools view -h -bS -o ${fullout}${prefix}.best.bam - ";
  my $command4 = "samtools sort -@ 8 -m 4G -O bam -o ${fullout}${prefix}.sorted.bam ${fullout}${prefix}.bam";
...
#  print $sh "pigz -f ${fullout}${prefix}.sam" . ' || { echo "gzip failed"; exit 1; }'."\n";
  print $sh "rm -f ${fullout}${prefix}.bam ${fullout}${prefix}.sam" . ' || { echo "rm failed"; exit 1; }'."\n";
