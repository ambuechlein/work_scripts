...
while(my $file = readdir($din)){
  next unless($file =~ /.featureCountsaccepted_hits.sorted.(r1.)*bam$/o);
  warn "Parsing $file\n";
  my $pre = $file;
  $pre =~ s/\.featureCountsaccepted_hits.(r1.)*bam//go;
  $pre =~ s/\.accepted_hits\S*\.bam//go;
  $pre =~ s/_thout//go;
  $pre =~ s/\.fastq.gz//go;
  if($file =~ /.featureCountsaccepted_hits.sorted.r1.bam/go){
    $pre .= ".r1";
    warn "$pre\n";
  }
  push(@samp,$pre);
