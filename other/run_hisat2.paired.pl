...
while(my $f = readdir($din)){
  ...
  my $pre = $f;
  $pre =~ s/_R1_001.paired.fastq.gz//g;
  my $command = "hisat2 -q --rna-strandness F --new-summary --summary-file $outdir/$pre.summary.tsv --un-gz $outdir/unmapped.tsv.gz -p 8 --mm -x $options{genome} -1 $file1 -2 $file2 -S $outdir/accepted_hits.sam";
